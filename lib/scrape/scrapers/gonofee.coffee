...
module.exports = new Scraper
  ...
  $ToListing: ($) ->
    rent: accounting.unformat _.trim $('#rightcolumn .stats tr:nth-child(5)').text().replace('Monthly Rent', '')
    beds: parseBeds _.trim $('#rightcolumn .stats tr:nth-child(7)').text().replace('Bedrooms', '')
    baths: parseInt _.trim $('#rightcolumn .stats tr:nth-child(8)').text().replace('Bathrooms', '')
    location:
